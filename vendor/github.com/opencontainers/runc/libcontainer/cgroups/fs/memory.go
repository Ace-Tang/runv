			...
			if pathErr, ok := err.(*os.PathError); ok {
				if errNo, ok := pathErr.Err.(syscall.Errno); ok {
					if errNo == syscall.EBUSY {
						return fmt.Errorf("failed to set %s, because either tasks have already joined this cgroup or it has children %s %d", cgroupKernelMemoryLimit, path, kernelMemoryLimit)
					}
				}
			}
...
func (s *MemoryGroup) Set(path string, cgroup *configs.Cgroup) error {
	...
	if cgroup.Resources.MemorySwappiness != nil {
		if v := *cgroup.Resources.MemorySwappiness; v >= 0 && v <= 100 {
			if err := writeFile(path, "memory.swappiness", strconv.FormatInt(v, 10)); err != nil {
				return err
			}
		}
	}
